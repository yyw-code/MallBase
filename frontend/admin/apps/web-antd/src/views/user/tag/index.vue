...
const { tableData, loading, pagination, loadData, handleDelete } = useTableCrud<
  UserTagApi.TagItem,
  UserTagApi.ListParams
>(
  {
    delete: deleteUserTagApi,
    list: getUserTagListApi,
  },
  { immediateLoad: false },
);
...
const editingItem = ref<null | UserTagApi.TagItem>(null);
...
const handleEdit = async (record: UserTagApi.TagItem) => {
  try {
    const detail = await getUserTagInfoApi(record.id);
...
const handleStatusChange = async (
  record: UserTagApi.TagItem,
  checked: boolean,
) => {
  try {
    await updateUserTagStatusApi(record.id, checked ? 1 : 0);
...
/* ---------------- 查看用户数 ---------------- */
const handleViewUsers = async (record: UserTagApi.TagItem) => {
  try {
    const result = await getUserTagCountApi(record.id);
    Modal.info({
      title: '标签用户数',
      content: `「${record.name}」标签下共有 ${result.count} 个用户`,
    });
  } catch (error) {
...
<template>
  <div class="p-4">
    <div class="mb-3 flex items-center justify-between gap-4">
      <h2 class="m-0 text-lg font-semibold">用户标签</h2>
      <div class="flex flex-wrap justify-end gap-2">
        <a-button
          type="primary"
          @click="handleCreate"
          v-access:code="'SystemUserTagCreate'"
        >
          新增标签
        </a-button>
          ...
          <template v-if="column.key === 'action'">
            <a-space>
              <a-button
                ...
                v-access:code="'SystemUserTagUpdate'"
              >
                编辑
              </a-button>
              <a-button
                ...
                v-access:code="'SystemUserTagGetUserTags'"
              >
                查看用户
              </a-button>
              <a-button
