...
import type { SkuRow } from './composables/useGoodsEdit';
import type {
  GoodsEditMarketingSlot,
  GoodsEditSlotField,
...
import { useGoodsEdit } from './composables/useGoodsEdit';
import {
...
const distributionSecondLevelEnabled = ref(false);
...
const goodsEditSlotContext = computed(() => ({
  features: {
    distribution: distributionEnabled.value,
    distributionSecondLevel: distributionSecondLevelEnabled.value,
...
const onSubmit = () =>
  handleSubmit(() => router.back(), goodsEditSlotContext.value.features);
...
const skuDetailEditingRow = ref<null | SkuRow>(null);
...
const getMarketingSlotDescription = (slot: GoodsEditMarketingSlot) =>
  slot.description?.(goodsEditSlotContext.value) || '';
...
async function loadMarketingConfig() {
  ...
  if (distributionResult.status === 'fulfilled') {
    const items = settingItems(distributionResult.value);
    const distributionSwitch = items.find(
      (item) => item.code === 'distribution_enabled',
    );
    const secondLevelSwitch = items.find(
      (item) => item.code === 'second_level_enabled',
    );
    distributionEnabled.value = settingSwitchEnabled(
      distributionSwitch?.value,
      true,
    );
    distributionSecondLevelEnabled.value = settingSwitchEnabled(
      secondLevelSwitch?.value,
      false,
    );
  } else {
    distributionEnabled.value = true;
    distributionSecondLevelEnabled.value = false;
  }
}
  ...
  <div class="goods-edit-page" :class="{ fullscreen: isFullscreen }">
              ...
              <a-tab-pane key="basic" tab="基本信息">
                <div class="tab-body">
                  ...
                  <a-form-item label="运费模板">
                    <a-select
                      v-model:value="formData.freight_template_id"
                      placeholder="不选则视为包邮"
                      allow-clear
                      style="width: 240px"
                    >
                      <a-select-option
                        v-for="t in freightTemplateOptions"
                        :key="t.id"
                        :value="t.id"
                      >
                        {{ t.name }}
                  ...
                  <a-form-item label="主视频">
                    <div class="form-tip">
                      可选，建议上传 9-30 秒商品展示短视频（MP4）
                    </div>
                    <Upload
                      v-model:value="formData.main_video"
                      type="video"
                      module="goods"
                    />
                  </a-form-item>
                  ...
                  <a-form-item label="商品标签">
                    <div v-if="tagOptions.length === 0" class="form-tip">
                      暂无标签，请先在「商品标签」模块创建
                    </div>
                    <div v-else class="tag-list">
                      <span
                        ...
                        :style="
                          formData.tag_ids.includes(tag.id) && tag.color
                            ? {
                        ...
                        @click="
                          formData.tag_ids.includes(tag.id)
                            ? (formData.tag_ids = formData.tag_ids.filter(
                                (id) => id !== tag.id,
                              ))
                      ...
                      >
                        <span
                          v-if="tag.color && !formData.tag_ids.includes(tag.id)"
                          class="tag-dot"
                ...
                <div class="tab-body">
                  ...
                  >
                    ...
                    >
                      ...
                      <div
                        v-if="getMarketingSlotDescription(slot)"
                        class="marketing-slot-description form-tip"
                      >
                        {{ getMarketingSlotDescription(slot) }}
                      </div>
                    </a-form-item>
              ...
              <a-tab-pane key="spec" tab="规格库存">
                <div class="tab-body">
                  ...
                  <a-form-item v-if="specType === 'multi'" label="规格详情">
                    ...
                    />
                    <span class="form-tip ml8"
                      >开启后，前台优先展示用户选中规格的详情；未设置时回退商品通用详情</span
                    >
                  </a-form-item>
                  <a-alert
                    v-if="
                      formData.sku_detail_enabled === 1 && specType === 'multi'
                    "
                    class="mb12"
                  ...
                  <template v-else>
                        ...
                        <div ref="specListRef" class="spec-list">
                          ...
                          >
                            <div class="spec-name-row">
                              ...
                              <div class="spec-name-actions">
                                <a-checkbox
                                  ...
                                  @change="
                                    (e: any) => {
                                      attr.add_pic = e.target.checked ? 1 : 0;
                                      toggleAddPic(e.target.checked, attrIdx);
                                    }
                                  "
                                >
                                  规格图
                                ...
                                <a-button
                                  ...
                                  danger
                                  size="small"
                                  @click="handleRemoveSpec(attrIdx)"
                                >
                                  删除
                                </a-button>
                              </div>
                            ...
                            <div class="spec-item-meta">
                              ...
                              <span v-if="attr.add_pic === 1"
                                >已开启规格图，图片可同步替换命中 SKU</span
                              >
                              <span v-else
                                >纯文字规格，规格值支持拖动调整顺序</span
                              >
                            </div>
                            <div
                              :ref="
                                (el) => {
                                  valueListRefs[attrIdx] = el as HTMLElement;
                                }
                              "
                            ...
                            >
                              <template v-if="attr.add_pic">
                                <div
                                  ...
                                  :key="det.id"
                                  class="spec-val-item has-pic"
                                >
                                  <div class="spec-val-media-top">
                                    <span class="val-drag-handle"
                                      ><svg
                                        width="10"
                                        height="10"
                                        viewBox="0 0 12 12"
                                    ...
                                    ></span>
                                    <span
                                      class="val-del media-del"
                                      @click="removeSpecValue(attrIdx, detIdx)"
                                      >×</span
                                    >
                                  </div>
                                  <div class="val-pic-cell media-card">
                                    <div
                                      v-if="det.pic"
                                      class="val-pic-thumb media-thumb"
                                    >
                                    ...
                                    <div
                                      v-else
                                      class="val-pic-upload-wrap media-thumb media-upload-wrap"
                                    >
                                      <Upload
                                        type="image"
                                        module="goods"
                                  ...
                                  <a-input
                                    ...
                                    placeholder="规格值"
                                    :maxlength="30"
                                    class="val-input media-input"
                                    @change="generateSkuCombinations"
                                  />
                                </div>
                                <button
                                  type="button"
                                  class="add-val-card add-val-card-pic"
                                  @click="addSpecValue(attrIdx)"
                                >
                    ...
                    >
                      <div class="sku-panel">
                        <div class="sku-panel-head">
                          <div>
                            ...
                            <div class="form-tip">
                              批量编辑可先按规格筛选，再对命中的 SKU 统一处理
                            </div>
                          </div>
                          <div class="sku-panel-summary">
                            已生成 {{ skuRows.length }} 个 SKU
                          </div>
                        </div>
                        <div class="sku-batch-toolbar">
                          <div class="sku-batch-actions">
                            ...
                            <button
                              type="button"
                              class="light-tool-btn primary"
                              @click="applyBatch(goodsEditSlotContext.features)"
                            >
                              批量修改 / 快速清空
                            </button>
                            <button
                          ...
                          >
                            <template #bodyCell="{ column }">
                              <template v-if="(column as any)._isSpecCol">
                                <a-select
                                  v-model:value="
                                    batchFilters[(column as any).title]
                                  "
                                  ...
                                  <a-select-option
                                    v-for="attr in attrs
                                      .find(
                                        (item) =>
                                          (item.value ||
                                            `规格${attrs.indexOf(item) + 1}`) ===
                                          (column as any).title,
                                      )
                                      ?.detail.filter((item) => item.value) ||
                                    []"
                                    :key="attr.id"
                                    :value="attr.value"
                                  >
                              ...
                              <template
                                v-else-if="column.dataIndex === 'image'"
                              >
                                <div class="batch-image-cell">
                                  <div
                                    class="batch-image-editor"
                                    title="批量设置图片"
                                  >
                                    <Upload
                                      v-model:value="
                                        (batchData as any).__image__
                                      "
                                      type="image"
                                      module="goods"
                                      :show-upload-list="false"
                              ...
                              >
                                <a-input-number
                                  v-model:value="(batchData as any).__price__"
                                  placeholder="批量售价"
                                  :min="0"
                                  :precision="2"
                                  size="small"
                              ...
                              <template
                                v-else-if="column.dataIndex === 'market_price'"
                              >
                                <a-input-number
                                  v-model:value="
                                    (batchData as any).__market_price__
                                  "
                                  placeholder="批量市价"
                                  :min="0"
                                  :precision="2"
                              ...
                              <template
                                v-else-if="column.dataIndex === 'stock'"
                              >
                                <a-input-number
                                  v-model:value="(batchData as any).__stock__"
                                  placeholder="批量库存"
                                  :min="0"
                                  size="small"
                                  :controls="false"
                              ...
                              <template
                                v-else-if="column.dataIndex === 'sku_code'"
                              >
                                <a-input
                                  v-model:value="batchData.__sku_code__"
                                  placeholder="批量SKU编码"
                                  size="small"
                                  class="batch-cell-control"
                                />
                              ...
                              <template
                                v-else-if="getSkuExtensionField(column)"
                              >
                                <a-select
                                  v-if="
                                    getSkuExtensionField(column)?.type ===
                                    'select'
                                  "
                                  v-model:value="
                                ...
                                >
                                  <a-select-option
                                    v-for="option in getSkuExtensionField(
                                      column,
                                    )?.options || []"
                                    :key="option.value"
                                    :value="option.value"
                                ...
                                >
                                  <template
                                    v-if="getSkuExtensionField(column)?.prefix"
                                    #prefix
                                  >
                              ...
                              <template
                                v-else-if="column.dataIndex === '_action'"
                              >
                                <a-select
                                  v-model:value="(batchData as any).__is_show__"
                                  placeholder="显示状态"
                                  size="small"
                                  allow-clear
                                  class="batch-cell-control"
                      ...
                      <a-table
                        :columns="visibleSkuColumns as any[]"
                        ...
                        bordered
                        row-key="spec_values"
                        class="sku-table compact-table"
                      >
                        <template #bodyCell="{ column, record }">
                          <template v-if="(column as any)._isSpecCol">
                            <span class="sku-spec-val">{{
                              (record as SkuRow).detail[(column as any).title]
                            }}</span>
                          </template>
                          <template v-else-if="column.dataIndex === 'image'">
                            <div class="sku-img-cell">
                              <div
                                v-if="getSkuPreviewImage(record as SkuRow)"
                                class="sku-img-thumb"
                              >
                                <img
                                  :src="
                                    getPicPreviewUrl(
                                      getSkuPreviewImage(record as SkuRow)!,
                                    )
                                  "
                                />
                                <span
                                  v-if="(record as SkuRow).image"
                                  class="sku-img-del"
                                  @click.stop="
                                    (record as SkuRow).image = undefined
                                  "
                                  >×</span
                                >
                              </div>
                          ...
                          <template v-else-if="column.dataIndex === 'price'">
                            <a-input-number
                              v-model:value="(record as SkuRow).price"
                              :min="0"
                              :precision="2"
                              size="small"
                              :controls="false"
                          ...
                          >
                            <a-input-number
                              v-model:value="(record as SkuRow).market_price"
                              :min="0"
                              :precision="2"
                              size="small"
                              :controls="false"
                          ...
                          <template v-else-if="column.dataIndex === 'stock'">
                            <a-input-number
                              v-model:value="(record as SkuRow).stock"
                              :min="0"
                              size="small"
                              :controls="false"
                              style="width: 100%"
                          ...
                          <template v-else-if="column.dataIndex === 'sku_code'">
                            <a-input
                              v-model:value="(record as SkuRow).sku_code"
                              size="small"
                              placeholder="选填"
                              allow-clear
                            />
                          ...
                          <template v-else-if="getSkuExtensionField(column)">
                            <a-select
                              v-if="
                                getSkuExtensionField(column)?.type === 'select'
                              "
                              v-model:value="
                            ...
                            >
                              <a-select-option
                                v-for="option in getSkuExtensionField(column)
                                  ?.options || []"
                                :key="option.value"
                                :value="option.value"
                              >
                          ...
                          >
                            <a-button
                              size="small"
                              type="link"
                              @click="openSkuDetailEditor(record as SkuRow)"
                            >
                              {{
                                (record as SkuRow).description
                                  ? '已设置'
                                  : '编辑'
                              }}
                            </a-button>
    ...
    >
      <a-spin :spinning="specLibLoading">
        <a-tabs v-model:active-key="specImportTab">
          <a-tab-pane key="spec" tab="规格库">
            <div
              v-if="specLibList.length === 0 && !specLibLoading"
              class="empty-tip"
            >
              规格库暂无数据，请先在「商品规格」模块创建
            </div>
            <a-checkbox-group
              v-else
          ...
          <a-tab-pane key="template" tab="规格模板">
            <div
              v-if="specTemplateList.length === 0 && !specLibLoading"
              class="empty-tip"
            >
              暂无模板，可在「规格模板」模块创建
            </div>
            ...
            >
              <div
                v-for="tpl in specTemplateList"
                :key="tpl.id"
                class="spec-lib-item"
              >
                <a-checkbox :value="tpl.id">
                  <span class="spec-lib-name">{{ tpl.name }}</span>
                </a-checkbox>
                <div class="spec-lib-values">
                  <span
                    v-for="item in tpl.detail || []"
                    :key="item.spec_name"
                    class="spec-lib-val-tag"
                    >{{ item.spec_name }}（{{ item.values.length }}个值）</span
                  >
                </div>
              </div>
            </a-checkbox-group>
    ...
    >
      ...
      <div class="save-template-list">
        ...
        >
          ...
          <div class="spec-lib-values">
            <span v-for="v in item.values" :key="v" class="spec-lib-val-tag">{{
              v
            }}</span>
            <span v-if="item.values.length === 0" class="form-tip"
              >（无规格值，不可保存）</span
            >
          </div>
        </div>
    ...
    <a-modal
      ...
      :title="
        skuDetailEditingRow
          ? `规格详情 - ${skuDetailEditingRow.spec_values}`
          : '规格详情'
      "
      :width="900"
      ok-text="保存"
...
.media-thumb,
.media-thumb :deep(.ant-upload.ant-upload-select) {
  width: 100% !important;
...
.val-pic-upload-wrap :deep(.ant-upload-wrapper) {
  width: 100% !important;
  height: 64px !important;
  min-width: 100% !important;
  min-height: 64px !important;
}
.val-pic-upload-wrap :deep(.ant-upload-text) {
  display: none !important;
}
.val-pic-upload-wrap :deep(.ant-upload-list) {
  display: none !important;
}
.media-upload-wrap :deep(.ant-upload) {
...
.light-tool-btn.primary {
  background: hsl(var(--primary) / 0.08);
  border-color: hsl(var(--primary) / 0.24);
  color: hsl(var(--primary));
}
.light-tool-btn:disabled {
  opacity: 0.55;
  cursor: not-allowed;
}
...
.batch-image-editor :deep(.ant-upload.ant-upload-select-picture-card) {
  width: 100% !important;
...
.batch-image-editor :deep(.ant-upload) {
  display: flex;
  align-items: center;
  justify-content: center;
  width: 100% !important;
...
.batch-image-editor :deep(.ant-upload-list) {
  display: none !important;
}
.sku-batch-table {
  min-width: max-content;
}
.sku-batch-table :deep(.ant-table-tbody > tr > td) {
  background: hsl(var(--card));
}
.sku-hit-stat {
  display: flex;
...
.sku-img-cell .sku-img-thumb {
  position: relative;
  display: inline-block;
}
.sku-img-cell .sku-img-thumb img {
  width: 36px;
  height: 36px;
  object-fit: cover;
  border-radius: 3px;
...
.sku-img-cell .sku-img-del:hover {
  background: #ff4d4f;
}
.sku-img-cell :deep(.ant-upload-list) {
  display: none !important;
}
.sku-img-cell :deep(.ant-upload.ant-upload-select) {
...
.sku-img-cell :deep(.ant-upload-text) {
  display: none !important;
}
.sku-img-cell :deep(.ant-upload-wrapper) {
  display: block;
  width: 36px;
  height: 36px;
}
