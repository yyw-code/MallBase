...
CREATE TABLE `mb_goods_category` (
  ...
  `image` bigint(20) unsigned DEFAULT NULL COMMENT '分类图片素材ID',
...
CREATE TABLE `mb_goods_brand` (
  ...
  `logo` bigint(20) unsigned DEFAULT NULL COMMENT '品牌LOGO素材ID',
...
CREATE TABLE `mb_goods` (
  ...
  `freight_template_id` int(11) unsigned DEFAULT NULL COMMENT '运费模板ID（空=包邮）',
  `name` varchar(200) NOT NULL COMMENT '商品名称',
  `subtitle` varchar(255) DEFAULT NULL COMMENT '商品副标题',
  `main_image` bigint(20) unsigned DEFAULT NULL COMMENT '主图素材ID',
  `main_video` bigint(20) unsigned DEFAULT NULL COMMENT '主视频素材ID',
  ...
  KEY `idx_freight_template_id` (`freight_template_id`),
...
CREATE TABLE `mb_goods_comment` (
  ...
  `order_item_id` int(11) unsigned DEFAULT NULL COMMENT '订单项ID',
  `sku_id` int(11) unsigned DEFAULT NULL COMMENT 'SKU ID',
  `sku_spec` varchar(500) DEFAULT NULL COMMENT 'SKU规格文案快照',
  `content` text NOT NULL COMMENT '评论内容',
  `images` json DEFAULT NULL COMMENT '评论图片素材ID数组 JSON',
  `append_content` text DEFAULT NULL COMMENT '追评内容',
  `append_images` json DEFAULT NULL COMMENT '追评图片素材ID数组 JSON',
