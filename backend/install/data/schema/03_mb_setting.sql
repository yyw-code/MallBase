...
(105, '客服方式', 'client_customer_service_mode', 'phone', 'radio', '[{"label":"手机号客服","value":"phone"},{"label":"在线客服系统","value":"system"}]', '[{"type":"required","message":"请选择客服方式"}]', NULL, '控制客户端“联系客服”入口使用手机号或在线客服系统', 72),
...
(105, '客服 Widget 地址', 'customer_service_widget_url', '', 'input', NULL, NULL, 'https://customer.example.com/widget', '客户端打开在线客服的前端地址', 77),
(105, '客服 API 地址', 'customer_service_api_base', '', 'input', NULL, NULL, 'https://customer.example.com', '客服系统 API 地址，供客户端客服页按需使用', 78),
(105, '客服 Socket 地址', 'customer_service_socket_base', '', 'input', NULL, NULL, 'wss://customer.example.com', '客服系统 WebSocket 地址，供客户端客服页按需使用', 79),
(105, '客服平台标识', 'customer_service_platform_code', 'mallbase', 'input', NULL, '[{"type":"required","message":"请输入平台标识"},{"type":"pattern","value":"^[a-z0-9][a-z0-9_-]{1,31}$","message":"平台标识须以小写字母或数字开头，只能包含小写字母、数字、下划线和短横线"}]', 'mallbase', '写入客服上下文 Token 的平台标识，须与 Customer Service 平台编码一致', 80),
(105, '上下文 Key ID', 'customer_service_context_key_id', '', 'input', NULL, '[{"type":"pattern","value":"^ctx_[A-Za-z0-9_-]{20,64}$","message":"上下文 Key ID 格式不正确"}]', 'ctx_...', '系统客服模式下必填；由 Customer Service 创建上下文凭证后一次性提供，与上下文签名密钥成对使用', 81),
(105, '上下文 Token 有效期', 'customer_service_context_ttl', '300', 'number', NULL, '[{"type":"required","message":"请填写 Token 有效期"},{"type":"integer","message":"Token 有效期必须是整数"},{"type":"min","value":60,"message":"不能小于 60 秒"},{"type":"max","value":300,"message":"不能大于 300 秒"}]', NULL, '单位秒，Customer Service 当前协议最多允许 300 秒', 82),
(105, '上下文签名密钥', 'customer_service_context_secret', '', 'password', NULL, '[{"type":"min_length","value":32,"message":"上下文签名密钥至少 32 位"}]', '粘贴 Customer Service 一次性显示的密钥', '由 Customer Service 创建上下文凭证后一次性提供，用于 MallBase 签发短期上下文 Token', 83),
(105, '启用连接器', 'customer_service_connector_enabled', '0', 'switch', NULL, NULL, NULL, '关闭后客服系统不能调用 MallBase 连接器接口', 84),
(105, '连接器签名密钥', 'customer_service_connector_secret', '', 'password', NULL, '[{"type":"min_length","value":32,"message":"连接器签名密钥至少 32 位"}]', '请输入不少于 32 位的随机密钥', '客服系统调用 MallBase 连接器接口的 HMAC 密钥', 85),
(105, '连接器签名窗口', 'customer_service_timestamp_window', '300', 'number', NULL, '[{"type":"required","message":"请填写签名时间窗口"},{"type":"integer","message":"签名时间窗口必须是整数"},{"type":"min","value":60,"message":"不能小于 60 秒"},{"type":"max","value":3600,"message":"不能大于 3600 秒"}]', NULL, '单位秒，超过窗口的连接器请求会被拒绝', 86),
(105, '连接器允许 IP', 'customer_service_allowed_ips', '', 'input', NULL, NULL, '127.0.0.1,10.0.0.2', '留空表示不限制；多个 IP 用英文逗号分隔', 87),
(105, '连接器操作管理员 ID', 'customer_service_operator_admin_id', '1', 'number', NULL, '[{"type":"required","message":"请填写操作管理员 ID"},{"type":"integer","message":"操作管理员 ID 必须是整数"},{"type":"min","value":1,"message":"操作管理员 ID 不能小于 1"}]', NULL, '客服系统操作订单、售后时用于后台日志审计的管理员 ID', 88),
