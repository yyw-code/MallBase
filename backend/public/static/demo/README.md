...
- 源：`backend/install/static/demo/`（即本目录）
- 目标：`backend/public/static/demo/`
- 策略：目标已存在同名文件 → 跳过；不存在 → 拷贝并设 `0644`。
- 错误：单文件失败仅记录到步骤详情，不阻断安装。

各安装方式都汇聚到 `InstallService::executeInstallation()`，因此本目录只维护一份即可：

| 安装方式 | 入口 | 是否走本目录 |
|---|---|---|
| Web 向导 | `InstallController::execute` | 是 |
| `php think install:auto` CLI | `app/command/InstallAuto.php` | 是 |
| Docker 仅后端 | 容器内进 Web 向导/命令 | 是 |
| Docker 全套 | `install-auto` 容器 | 是 |
| Docker 生产 | 镜像中执行 install:auto | 是，Dockerfile 复制 `backend/` 时会带上本目录。 |

## SQL 引用对应关系

`backend/install/data/demo/*.sql` 引用 `/static/demo/<file>`，只有启用演示数据时才会拷贝并使用本目录素材。
...
- `backend/install/data/demo/00_demo_upload_assets.sql`
- `backend/install/data/demo/04_demo_reviews.sql`
- `backend/install/data/demo/06_demo_recharge_package.sql`
