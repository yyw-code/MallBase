...
2. 执行 `sh deploy/docker/host-preflight.sh` 准备宿主机持久目录
3. 再执行 `docker compose up -d --build` 启动 HTTP、Queue、Cron 三个业务角色
4. Nginx 的 `/admin/api/`、`/install`、`/client/api/`、`/uploads/` 指向宿主机暴露的 `127.0.0.1:8080`
5. 执行 `nginx -t && systemctl reload nginx`
